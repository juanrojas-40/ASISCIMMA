            ...
            with col1:
                if st.button("💾 Guardar Asistencia", type="primary", use_container_width=True):
                    if sheets_manager.save_attendance(curso, fecha, asistencia, st.session_state.user):
                        ...
                        # === CORRECCIÓN PRINCIPAL: Calcular estadísticas y enviar ===
                        if st.checkbox("📧 Enviar notificaciones a apoderados"):
                            with st.spinner("Preparando datos y enviando emails..."):
                                # Cargar curso con asistencia histórica
                                cursos_sede = sheets_manager.load_courses_by_sede(st.session_state.sede)
                                curso_completo = cursos_sede.get(curso, {})
                                
                                # Calcular stats para cada estudiante
                                student_stats = {}
                                total_fechas = len(curso_data["fechas"])
                                for est in curso_data["estudiantes"]:
                                    est_key = est.strip().lower()
                                    asist_hist = curso_completo.get("asistencias", {}).get(est, {})
                                    # Incluir la asistencia del día actual
                                    asist_today = asistencia.get(est, False)
                                    todas_asist = asist_hist.copy()
                                    todas_asist[fecha] = asist_today
                                    
                                    presentes = sum(1 for v in todas_asist.values() if v)
                                    ausentes = total_fechas - presentes
                                    porc = (presentes / total_fechas * 100) if total_fechas > 0 else 0
                                    
                                    if porc < 70:
                                        rec = "Le recomendamos mejorar la asistencia para un mejor rendimiento académico."
                                    elif porc < 85:
                                        rec = "Su asistencia es buena, pero puede mejorar."
                                    else:
                                        rec = "¡Excelente asistencia! Continúe así."
                                    
                                    student_stats[est_key] = {
                                ...
                                # Enviar con los datos correctos
                                resultados = email_manager.send_attendance_emails(
                                    curso, fecha, asistencia, student_stats
                                )
...
def show_secretaria_dashboard(sheets_manager, email_manager):
            ...
            with st.spinner("Cargando cursos..."):
                ...
                if curso_seleccionado:
                    ...
                    with st.expander("📊 Información del Curso", expanded=True):
                        ...
                        with col3:
                            st.metric("👨‍🏫 Profesor", curso_data.get("profesor", "No asignado"))
                        with col4:
                            st.metric("📚 Asignatura", curso_data.get("asignatura", "No especificada"))
    ...
    with tab3:
        ...
        # === AGREGADO: Selector de fecha para el reporte del día ===
        st.markdown("### Paso 0: Seleccionar Fecha del Reporte")
        cursos_sede = sheets_manager.load_courses_by_sede(user_sede)
        if not cursos_sede:
        ...
        for curso_data in cursos_sede.values():
            all_fechas.update(curso_data.get("fechas", []))
        all_fechas = sorted(list(all_fechas), reverse=True)
        
        if not all_fechas:
            st.warning("No se encontraron fechas en los cursos.")
            return
        
        fecha_reporte = st.selectbox(
            "Selecciona la fecha para el reporte de asistencia:",
            all_fechas,
            key="fecha_reporte_sede"
        )
        # ======================================================
        ...
        asunto = st.text_input("Asunto del email:",
                               value=f"Información de Asistencia - Sede {user_sede} - {fecha_reporte}",
                               key="email_asunto")
        plantilla_base = f"""Estimado/a apoderado/a,
Le informamos sobre la situación de asistencia de {{estudiante}} en el curso {{curso}} de la sede {user_sede} en la fecha {fecha_reporte}.
